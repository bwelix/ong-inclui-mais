...
// Adicione esta função ao initMasks()
function initMasks() {
    // Máscara para CNPJ (já existente)
    const cnpjInput = document.getElementById('ong-cnpj');
    if (cnpjInput) {
        cnpjInput.addEventListener('input', function(e) {
            let value = e.target.value.replace(/\D/g, '');
    ...
    // Máscara para CPF (novo)
    const cpfInput = document.getElementById('vol-cpf');
    if (cpfInput) {
    ...
    // Máscara para Telefone (atualizada)
    const phoneInputs = document.querySelectorAll('input[type="tel"]');
    phoneInputs.forEach(input => {
        input.addEventListener('input', function(e) {
            let value = e.target.value.replace(/\D/g, '');
    ...
    // Máscara para CEP (nova)
    const cepInput = document.getElementById('vol-cep');
    if (cepInput) {
    ...
    // Buscar endereço automático via CEP (funcionalidade extra)
    if (cepInput) {
...
// Função para buscar endereço via API (funcionalidade extra)
function buscarEndereco(cep) {
    fetch(`https://viacep.com.br/ws/${cep}/json/`)
        .then(response => response.json())
        .then(data => {
            if (!data.erro) {
                document.getElementById('vol-address').value = data.logradouro;
                document.getElementById('vol-city').value = data.localidade;
                document.getElementById('vol-state').value = data.uf;
            }
        })
        .catch(error => {
...
// Atualize a função validateVoluntarioForm() para incluir os novos campos
function validateVoluntarioForm() {
    const nome = document.getElementById('vol-name');
    const email = document.getElementById('vol-email');
    const cpf = document.getElementById('vol-cpf');
    const telefone = document.getElementById('vol-phone');
    const nascimento = document.getElementById('vol-birthdate');
    const cep = document.getElementById('vol-cep');
    const endereco = document.getElementById('vol-address');
    const cidade = document.getElementById('vol-city');
    const estado = document.getElementById('vol-state');
    const habilidades = document.getElementById('vol-skills');
    const disponibilidade = document.getElementById('vol-availability');
    ...
    const camposObrigatorios = [
        { campo: nome, mensagem: 'Nome completo é obrigatório' },
        { campo: email, mensagem: 'E-mail é obrigatório' },
        { campo: cpf, mensagem: 'CPF é obrigatório' },
        { campo: telefone, mensagem: 'Telefone é obrigatório' },
        { campo: nascimento, mensagem: 'Data de nascimento é obrigatória' },
        { campo: cep, mensagem: 'CEP é obrigatório' },
        { campo: endereco, mensagem: 'Endereço é obrigatório' },
        { campo: cidade, mensagem: 'Cidade é obrigatória' },
        { campo: estado, mensagem: 'Estado é obrigatório' },
        { campo: habilidades, mensagem: 'Habilidades são obrigatórias' },
        { campo: disponibilidade, mensagem: 'Disponibilidade é obrigatória' }
    ];
    ...
    if (cpf.value && !validateCPF(cpf.value.replace(/\D/g, ''))) {
    ...
    if (nascimento.value) {
        const nascimentoDate = new Date(nascimento.value);
        const hoje = new Date();
        ...
        if (idade < 18) {
            showError(nascimento, 'Você deve ter pelo menos 18 anos para se cadastrar');
            isValid = false;
        }
    }
